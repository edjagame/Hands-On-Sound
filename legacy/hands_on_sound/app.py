...
from hands_on_sound.gesture_model import GESTURE_TO_SOUND, load_gesture_model, predict_gesture
from legacy.hands_on_sound.paths import ACTIVE_MODEL_PATH
from legacy.hands_on_sound.sound_player import SoundPlayer
...
def main():
    global model, model_feature_mean, model_feature_std, model_class_names, mp_hands, hands, mp_draw

    selected_settings = settings()

    # Load the gesture prediction model selected by the app.
    model, metadata = load_gesture_model(str(ACTIVE_MODEL_PATH))
    model_feature_mean = metadata["feature_mean"]
    model_feature_std = metadata["feature_std"]
    model_class_names = metadata["class_names"]
        ...
        if current_frame % prediction_frequency == 0:
            ...
            if hand_data:
                for i, hand in enumerate(hand_data):
                    lms = hand[0]
                    handedness = hand[1]
                    gesture, confidence = predict_gesture(model, lms, model_feature_mean, model_feature_std, model_class_names)
        ...
        if current_predictions:
            for i, prediction in enumerate(current_predictions):
                if prediction:
                    ...
                    sound_key = GESTURE_TO_SOUND.get(gesture, "generic")
                    sp.current_sounds[i] = sp.sounds[sound_key]
                    sp.set_volume(sp.channels[i], center_y_list[i], height)
                    cv.putText(
                        black_img,
                        f'{"Left" if i == 0 else "Right"} Hand: {gesture}',
                        (20, 30 * (i+1)),
                        cv.FONT_HERSHEY_PLAIN,
                        3,
                        (255-i*128,128,128+i*128),
                        thickness=2,
                    )   
                    idle_frames[i] = 0
